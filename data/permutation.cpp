...
Permute::Permute() : sto(std::random_device{}()), bins_built(false) {}

Permute::Permute(int seed) : sto(seed), bins_built(false) {}

void Permute::generate_permutations(
    std::shared_ptr<std::vector<std::vector<int8_t>>> permutations,
...
void Permute::build_bins(arma::colvec &odds, double epsilon) {
...
void Permute::permute_thread(
    std::shared_ptr<std::vector<std::vector<int8_t>>> p, int ncases, int offset,
    int nperm, int seed) {
 ...
 */
std::vector<int8_t> Permute::unpack(int successes, int bin_size, bool shuffle,
                                    StochasticLib3 &rng) {
...
Permute::epsilon_permutation(int nperm, arma::vec &odds, arma::uword ncases,
