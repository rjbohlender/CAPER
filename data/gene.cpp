...
void Gene::parse(std::stringstream &ss, arma::vec &phenotypes) {
    ...
    for (const auto &ts : transcripts_) {
        ...
        arma::sp_mat gt_control(controls.n_elem, genotypes_[ts].n_cols);
        arma::uword k = 0;
        for (const auto &v : controls) { // Filter only on variables that are too common in controls
            gt_control.row(k) = genotypes_[ts].row(v);
            k++;
        }
