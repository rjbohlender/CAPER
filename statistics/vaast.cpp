...
VAASTLogic::VAASTLogic(arma::sp_mat X_, arma::vec &Y_, arma::vec &weights,
  ...
  if (group_threshold > 0 && X.n_cols > 1) {
    ...
    for (arma::uword i = 0; i < X.n_cols; i++) {
      ...
      double new_score = Score();
      expanded_scores(i) =
          (score - new_score >= 0) ? score - new_score : 0; // Calculate score
    }
  } else {
    expanded_scores = vaast_site_scores;
  }
...
double VAASTLogic::Score(const arma::sp_mat &X, const arma::vec &Y,
                         const arma::vec &w) {
  if (arma::accu(X) == 0) {
    return 0;
  }
  ...
  if (biallelic) {
    // Via email:
    // The idea is to add an additional term to the VAAST CLRT that captures the
    // frequency of biallelic variants in cases and controls. For starters, we
    // can try collapsing all variants with a positive VAAST score into one
    // biallelic term.
    arma::vec log_lh = LRT();
    ...
    variant(arma::find(variant > 1))
        .fill(1); // Set all compound hets and homozygous rare carriers to 1

    case_allele1 = variant.t() * Y;
    control_allele1 = variant.t() * (1 - Y);
    case_allele0 = n_case - case_allele1;
    control_allele0 = n_control - control_allele1; // Not 2 * because we're only
                                                   // allowing presence absence.
...
arma::vec VAASTLogic::LRT() {
  arma::vec alt_control_freq =
      control_allele1 / (control_allele0 + control_allele1);
  arma::vec alt_case_freq = case_allele1 / (case_allele0 + case_allele1);

  // Match -r behavior for VAAST
  alt_control_freq(arma::find(alt_control_freq > soft_maf_filter))
      .fill(soft_maf_filter);

  arma::vec null_freq =
      (case_allele1 + control_allele1) /
      (case_allele0 + case_allele1 + control_allele0 + control_allele1);

  arma::vec alt_control =
      log_likelihood(alt_control_freq, control_allele0, control_allele1);
  arma::vec alt_case =
  ...
  arma::vec null_control =
      log_likelihood(null_freq, control_allele0, control_allele1);
...
arma::vec VAASTLogic::log_likelihood(arma::vec &freq, arma::vec &allele0,
                                     arma::vec &allele1) {
...
void VAASTLogic::variant_grouping(const arma::sp_mat &X, const arma::vec &Y,
  ...
  for (int i = 0; i < positions.size(); i++) {
    ...
    variants.emplace_back(Variant(case_allele1(i), case_allele0(i),
                                  control_allele1(i), control_allele0(i), w(i),
                                  splitter[5], loc.str(), i, soft_maf_filter));
  }
  ...
  if (!printed_mergeinfo) {
    std::stringstream ss;
    ss << "/Users/rjbohlender/CLionProjects/PA_Test/aas_no_intron/" << k
       << ".mergeinfo";
  ...
  std::stable_sort(variants.begin(), variants.end(), [&](auto &a, auto &b) {
    if (w(a.index) == w(b.index)) {
      if (a.score == b.score) {
        return positions[a.index] >
               positions[b.index]; // Lexicographically sorted to match default
                                   // VAAST behavior
      } else {
  ...
  for (int i = 0; i < merging_vars.size(); i++) {
    ...
    // std::cerr << "gid: " << sub_groups.size() << " " << "var: " <<
    // positions[type.second[i].index] << " score: " << type.second[i].score <<
    // std::endl;
  ...
  for (auto &group : sub_groups) {
    gid++;
    // std::cerr << "gid: " << gid << " group_size:" << group.size() <<
    // std::endl;
...
void VAASTLogic::legacy_grouping(const arma::sp_mat &X, const arma::vec &Y,
  ...
  for (int i = 0; i < positions.size(); i++) {
    ...
    if (type == "SNV") {
      SNVs.emplace_back(Variant(case_allele1(i), case_allele0(i),
                                control_allele1(i), control_allele0(i), w(i),
                                type, loc.str(), i, soft_maf_filter));
    } else if (type == "insertion") {
      insertion.emplace_back(Variant(
          case_allele1(i), case_allele0(i), control_allele1(i),
          control_allele0(i), w(i), type, loc.str(), i, soft_maf_filter));
    } else if (type == "deletion") {
      deletion.emplace_back(Variant(case_allele1(i), case_allele0(i),
                                    control_allele1(i), control_allele0(i),
                                    w(i), type, loc.str(), i, soft_maf_filter));
    } else if (type == "SPDA") {
      spda.emplace_back(Variant(case_allele1(i), case_allele0(i),
                                control_allele1(i), control_allele0(i), w(i),
                                type, loc.str(), i, soft_maf_filter));
    } else if (type == "complex_substitution") {
      mnp.emplace_back(Variant(case_allele1(i), case_allele0(i),
                               control_allele1(i), control_allele0(i), w(i),
                               type, loc.str(), i, soft_maf_filter));
    } else {
  ...
  for (auto &type : type_idx_map) {
    ...
    std::stable_sort(
        type.second.begin(), type.second.end(), [&](auto &a, auto &b) {
          if (w(a.index) == w(b.index)) {
            if (a.score == b.score) {
              return positions[a.index] >
                     positions[b.index]; // Lexicographically sorted to match
                                         // default VAAST behavior
            } else {
              return a.score > b.score;
            }
          } else {
...
Variant::Variant(std::string type, double soft_maf_filter)
    : type(std::move(type)), soft_maf_filter(soft_maf_filter) {}

Variant::Variant(double case_allele1, double case_allele0,
                 double control_allele1, double control_allele0, double weight,
                 std::string type, std::string loc, int index,
                 double soft_maf_filter)
    : case_allele1(case_allele1), case_allele0(case_allele0),
      control_allele1(control_allele1), control_allele0(control_allele0),
      weight(weight), type(std::move(type)), loc(std::move(loc)), index(index),
...
void Variant::calc_score() {
  ...
  if (case_allele0 * control_allele1 >
      control_allele0 * case_allele1) { // OR filter in VAAST
    score = 0;
    return;
  }

  double case_alt_freq = case_allele1 / (case_allele0 + case_allele1);
  double control_alt_freq =
      control_allele1 / (control_allele0 + control_allele1);

  control_alt_freq =
      control_alt_freq > soft_maf_filter ? soft_maf_filter : control_alt_freq;

  double null_freq =
      (case_allele1 + control_allele1) /
      (case_allele1 + case_allele0 + control_allele1 + control_allele0);

  double p_alt_case = case_allele1 * std::log(case_alt_freq) +
                      case_allele0 * std::log(1. - case_alt_freq);
  double p_alt_control = control_allele1 * std::log(control_alt_freq) +
                         control_allele0 * std::log(1. - control_alt_freq);
  double p_null_case = case_allele1 * std::log(null_freq) +
                       case_allele0 * std::log(1. - null_freq);
  double p_null_control = control_allele1 * std::log(null_freq) +
                          control_allele0 * std::log(1. - null_freq);
