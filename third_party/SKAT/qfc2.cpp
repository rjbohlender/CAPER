...
double QFC::exp1(double x) {
  return x < -50.0 ? 0.0 : exp(x);
}

double QFC::square(double x) {
  return x * x;
}

double QFC::cube(double x) {
  return x * x * x;
}
 ...
 * @return
 */
double QFC::log1(double x, bool first) {
  if (fabs(x) > 0.1) {
...
/**
 * @brief Find order of absolute values of lb.
 */
void QFC::order() {
  int j, k;
  double lj;
...
/**
 * @brief find bound on tail probability using mgf
 * @param u
 * @param cx cutoff point
 * @return error bound
 */

double QFC::errbd(double u, double *cx) {
...
double QFC::ctff(double accx, double *upn) {
...
double QFC::truncation(double u, double tausq) {
...
void QFC::findu(double *utx, double accx) {
  double u, ut;
  int i;
...
void QFC::integrate(int nterm, double interv, double tausq, bool mainx) {
...
/**
 * @brief coef of tausq in error when convergence factor of exp1(-0.5*tausq*u^2) is used when df is evaluated at x
 */
double QFC::cfe(double x) {
...
/**
 * @brief count number of calls to errbd, truncation, cfe
 */
void QFC::counter() {
  count++;
  if (count > lim)
